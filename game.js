...
function moveBear(e) {
  ...
  const KEYRIGHT = 39;
  //speed of the bear
  let bearSpeed = document.getElementById("speedBees").value;
...
function getRandomInt(max) {
  return Math.floor(Math.random() * max); // math.random gets a random number between 0 and 1(exclusive) and math.floor returns the largest integer nearest to the given number.
} // The math.random is multiplied by the max to generate a number within its range
